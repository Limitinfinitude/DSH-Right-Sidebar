...
//#endregion
//#region node_modules/@deepseek-ai/dsh-session/lib/index.js
			...
			*/
			const drive = () => {
				...
				driverRun = (async () => {
					try {
						for (;;) {
							const signal = new Promise((resolve$1) => {
								wake = resolve$1;
							});
			...
			const binding = (name$1) => async (rawArgs) => {
				...
				const outcome = await new Promise((resolve$1, reject) => {
					let parked;
					const settle = (result) => {
						resolve$1(result.isError ? {
							isError: true,
...
function workspaceRoots(registry) {
	return [...new Set([canonicalRoot(process.cwd()), ...registry.list().map((workspace) => canonicalRoot(workspace.path))])];
}
async function resolveWorkspaceFile(raw, roots, allowedExtensions) {
...
async function validatePublicationResource(publication, roots) {
	if (publication.kind === "link") return void 0;
	...
	if (!(await stat(file)).isFile()) throw new Error("published path must identify a regular file");
	return file;
}
...
function registerPublishTool(ctx, roots) {
	return ctx.tools.register(defineTool({
		...
		parameters: {
			workId: {
				...publicationProperties.workId,
				description: "Stable work identity, at most 64 characters."
			},
			workTitle: {
				...publicationProperties.workTitle,
				description: "Visible work title, at most 80 characters."
			},
			resultId: {
				...publicationProperties.resultId,
				description: "Stable result identity within the work, at most 64 characters."
			},
			label: {
				...publicationProperties.label,
				description: "Visible result label, at most 80 characters."
			},
			kind: {
				...publicationProperties.kind,
				description: "document or visual for a workspace file; link for a deployed application."
			},
			path: {
				...publicationProperties.path,
				description: "Workspace-relative or workspace-contained absolute path for document and visual results."
			},
			url: {
				...publicationProperties.url,
				description: "Absolute HTTP or HTTPS URL for link results."
			}
		},
		output: {
			schema: {
...
function registerFileRoute(webServer, roots) {
	return webServer.register({
		kind: "route",
		path: ROUTE_PATH,
		async handler(req, res) {
			const file = await resolveWorkspaceFile(new URL(req.url ?? "/", "http://localhost").searchParams.get("path") ?? "", roots(), ALLOWED_EXTENSIONS);
			if (file === null) {
...
//#endregion
export { apply, inject, name, registerFileRoute };