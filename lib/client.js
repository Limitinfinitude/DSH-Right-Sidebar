...
var _comment = /<!--(?:-?>|[\s\S]*?(?:-->|$))/;
var html$2 = edit("^ {0,3}(?:<(script|pre|style|textarea)[\\s>][\\s\\S]*?(?:</\\1>[^\\n]*\\n+|$)|comment[^\\n]*(\\n+|$)|<\\?[\\s\\S]*?(?:\\?>\\n*|$)|<![A-Z][\\s\\S]*?(?:>\\n*|$)|<!\\[CDATA\\[[\\s\\S]*?(?:\\]\\]>\\n*|$)|</?(tag)(?: +|\\n|/?>)[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|<(?!script|pre|style|textarea)([a-z][\\w-]*)(?:attribute)*? */?>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|</(?!script|pre|style|textarea)[a-z][\\w-]*\\s*>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$))", "i").replace("comment", _comment).replace("tag", _tag).replace("attribute", / +[a-zA-Z:_][\w.:-]*(?: *= *"[^"\n]*"| *= *'[^'\n]*'| *= *[^\s"'=<>`]+)?/).getRegex();
...
const MENTIONED_EXTENSION_PATTERN = [...AUTOMATIC_EXTENSIONS, ...EXPLICIT_EXTENSIONS].sort((left, right) => right.length - left.length).join("|");
...
function cleanMentionedPath(raw) {
	...
	if (/^[A-Za-z][A-Za-z\d+.-]*:/.test(path) && !/^[A-Za-z]:[\\/]/.test(path) && !isNetworkOutput(path)) return null;
...
function mentionedOutputPaths(assistantText$1) {
	const paths = /* @__PURE__ */ new Set();
	const add = (raw) => {
		const path = cleanMentionedPath(raw);
		if (path !== null) paths.add(path);
	};
	const plainPattern = new RegExp(`(?<![\\p{L}\\p{N}_])(?:[A-Za-z]:[\\\\/][^\\r\\n\`\"'<>|?*]+?\\.(?:${MENTIONED_EXTENSION_PATTERN})|(?:\\.{0,2}[\\\\/])?[A-Za-z0-9_@+.,()\\-]+(?:[\\\\/][A-Za-z0-9_@+.,()\\-]+)*\\.(?:${MENTIONED_EXTENSION_PATTERN}))(?=$|[\\s，。；：！？,;:!?)])`, "giu");
	for (const line of assistantText$1.split(/\r?\n/)) for (const match of line.matchAll(/https?:\/\/[^\s<>"'`]+/giu)) {
		const before = line.slice(0, match.index);
		if (/(?:输出|产物|文件|报告|图片|图表|下载|预览|查看|结果|链接|地址|url)[^。！？!?]{0,48}$/iu.test(before)) add(match[0]);
	}
	const tokens = marked.lexer(assistantText$1);
	...
	for (const path of paths) {
		...
		const related = output.findIndex((candidate) => {
			if (isNetworkOutput(path) || isNetworkOutput(candidate)) return path.toLowerCase() === candidate.toLowerCase();
...
const outputDockDefinition = {
	...
	update: (context, match) => {
		...
		if (match.event.type === "assistant/message") {
			const text$2 = assistantText(match.event);
			const released = mentionedConditionalOutputs([...context.state.pending.values()], text$2);
			const releasedNames = new Set(released.flatMap((entry) => [entry.path.replaceAll("\\", "/").toLowerCase(), basename(entry.path).toLowerCase()]));
			const discovered = mentionedOutputPaths(text$2).flatMap((path) => {
				const normalized$1 = path.replaceAll("\\", "/").toLowerCase();
				if (releasedNames.has(normalized$1) || releasedNames.has(basename(path).toLowerCase())) return [];
				const disposition = outputDisposition(path);
			...
			const published = [...new Map([...released, ...discovered].map((entry) => [entry.path, entry])).values()];
			if (published.length === 0) return context.state;
			const pending$1 = new Map(context.state.pending);
			for (const entry of published) pending$1.delete(entry.path);
			return {
				...context.state,
				pending: pending$1,
				produced: [...context.state.produced, ...published]
			};
		}
...
var require_lucide_react = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	const AlignLeft$1 = createLucideIcon("AlignLeft", [
		["path", {
			d: "M15 12H3",
			key: "6jk70r"
		}],
	...
	const ArrowDown$1 = createLucideIcon("ArrowDown", [["path", {
		d: "M12 5v14",
		key: "s699le"
	}], ["path", {
		d: "m19 12-7 7-7-7",
	...
	const ArrowUp$1 = createLucideIcon("ArrowUp", [["path", {
		d: "m5 12 7-7 7 7",
		key: "hav0vg"
	}], ["path", {
		d: "M12 19V5",
	...
	const Braces$1 = createLucideIcon("Braces", [["path", {
	...
	const ChevronLeft$1 = createLucideIcon("ChevronLeft", [["path", {
		d: "m15 18-6-6 6-6",
		key: "1wnfg3"
	}]]);
	...
	const ChevronsUpDown$1 = createLucideIcon("ChevronsUpDown", [["path", {
		d: "m7 15 5 5 5-5",
		key: "1hf1tw"
	}], ["path", {
		d: "m7 9 5-5 5 5",
	...
	const ExternalLink$2 = createLucideIcon("ExternalLink", [
		["path", {
			d: "M15 3h6v6",
			key: "1q9fwt"
		}],
	...
	const Eye$1 = createLucideIcon("Eye", [["path", {
	...
	const Maximize2$1 = createLucideIcon("Maximize2", [
		["polyline", {
	...
	const RefreshCw$1 = createLucideIcon("RefreshCw", [
		["path", {
	...
	const Scan$1 = createLucideIcon("Scan", [
		["path", {
			d: "M3 7V5a2 2 0 0 1 2-2h2",
			key: "aa7l1z"
		}],
	...
	const Search$2 = createLucideIcon("Search", [["circle", {
		cx: "11",
		cy: "11",
		r: "8",
		key: "4ej97u"
	...
	const ShieldCheck$1 = createLucideIcon("ShieldCheck", [["path", {
		d: "M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z",
		key: "oel41y"
	}], ["path", {
		d: "m9 12 2 2 4-4",
	...
	const WrapText$1 = createLucideIcon("WrapText", [
		["line", {
			x1: "3",
			x2: "21",
			y1: "6",
	...
	const ZoomIn$1 = createLucideIcon("ZoomIn", [
		["circle", {
			cx: "11",
			cy: "11",
			r: "8",
	...
	const ZoomOut$1 = createLucideIcon("ZoomOut", [
		["circle", {
			cx: "11",
			cy: "11",
			r: "8",
...
var import_lucide_react$3 = require_lucide_react();
	...
	*/
	(function(root, factory) {
		...
		function getWorkerBlob() {
			var URL$1 = global.URL || global.webkitURL || null;
			var code = moduleFactory.toString();
			return Papa$1.BLOB_URL || (Papa$1.BLOB_URL = URL$1.createObjectURL(new Blob([
		...
		var Papa$1 = {};
		Papa$1.parse = CsvToJson;
		Papa$1.unparse = JsonToCsv;
		Papa$1.RECORD_SEP = String.fromCharCode(30);
		Papa$1.UNIT_SEP = String.fromCharCode(31);
		Papa$1.BYTE_ORDER_MARK = "﻿";
		Papa$1.BAD_DELIMITERS = [
			"\r",
			"\n",
			"\"",
			Papa$1.BYTE_ORDER_MARK
		];
		Papa$1.WORKERS_SUPPORTED = !IS_WORKER && !!global.Worker;
		Papa$1.NODE_STREAM_INPUT = 1;
		Papa$1.LocalChunkSize = 1024 * 1024 * 10;
		Papa$1.RemoteChunkSize = 1024 * 1024 * 5;
		Papa$1.DefaultDelimiter = ",";
		Papa$1.Parser = Parser;
		Papa$1.ParserHandle = ParserHandle;
		Papa$1.NetworkStreamer = NetworkStreamer;
		Papa$1.FileStreamer = FileStreamer;
		Papa$1.StringStreamer = StringStreamer;
		Papa$1.ReadableStreamStreamer = ReadableStreamStreamer;
		if (typeof PAPA_BROWSER_CONTEXT === "undefined") Papa$1.DuplexStreamStreamer = DuplexStreamStreamer;
		...
		function CsvToJson(_input, _config) {
			...
			if (_config.worker && Papa$1.WORKERS_SUPPORTED) {
				var w = newWorker();
			...
			var streamer = null;
			if (_input === Papa$1.NODE_STREAM_INPUT && typeof PAPA_BROWSER_CONTEXT === "undefined") {
		...
		function JsonToCsv(_input, _config) {
			...
			function unpackConfig() {
				...
				if (typeof _config.delimiter === "string" && !Papa$1.BAD_DELIMITERS.filter(function(value) {
			...
			function safe(str, col) {
				...
				needsQuotes = needsQuotes || _quotes === true || typeof _quotes === "function" && _quotes(str, col) || Array.isArray(_quotes) && _quotes[col] || hasAny(escapedQuoteStr, Papa$1.BAD_DELIMITERS) || escapedQuoteStr.indexOf(_delimiter) > -1 || strValue.indexOf(_quoteChar) > -1 || escapedQuoteStr.charAt(0) === " " || escapedQuoteStr.charAt(escapedQuoteStr.length - 1) === " ";
		...
		function ChunkStreamer(config) {
			...
			function replaceConfig(config$1) {
				var configCopy = copy(config$1);
				configCopy.chunkSize = parseInt(configCopy.chunkSize);
				if (!config$1.step && !config$1.chunk) configCopy.chunkSize = null;
		...
		function NetworkStreamer(config) {
			config = config || {};
			if (!config.chunkSize) config.chunkSize = Papa$1.RemoteChunkSize;
			...
			function getFileSize(xhr$1) {
				var contentRange = xhr$1.getResponseHeader("Content-Range");
		...
		function FileStreamer(config) {
			config = config || {};
			if (!config.chunkSize) config.chunkSize = Papa$1.LocalChunkSize;
			ChunkStreamer.call(this, config);
			var reader, slice;
		...
		function ParserHandle(_config) {
			...
			*/
			this.parse = function(input, baseIndex, ignoreLastRow) {
				...
				if (!_config.delimiter) {
					...
					if (delimGuess.successful) _config.delimiter = delimGuess.bestDelimiter;
						...
						_config.delimiter = Papa$1.DefaultDelimiter;
					}
			...
			this.resume = function() {
				if (self$1.streamer._halted) {
					_paused = false;
					self$1.streamer.parseChunk(_input, true);
				} else setTimeout(self$1.resume, 3);
			};
			...
			function processResults() {
				if (_results && _delimiterError) {
					addError("Delimiter", "UndetectableDelimiter", "Unable to auto-detect delimiting character; defaulted to '" + Papa$1.DefaultDelimiter + "'");
					_delimiterError = false;
				}
			...
			function fillHeaderFields() {
				if (!_results) return;
				function addHeader(header, i$1) {
					header = stripBom(header);
					if (isFunction(_config.transformHeader)) header = _config.transformHeader(header, i$1);
					_fields.push(header);
				}
			...
			function guessDelimiter(input, newline$1, skipEmptyLines, comments, delimitersToGuess) {
		...
		function Parser(config) {
			...
			if (typeof delim !== "string" || Papa$1.BAD_DELIMITERS.indexOf(delim) > -1) delim = ",";
			if (comments === delim) throw new Error("Comment character same as delimiter");
			else if (comments === true) comments = "#";
			else if (typeof comments !== "string" || Papa$1.BAD_DELIMITERS.indexOf(comments) > -1) comments = false;
			if (newline$1 !== "\n" && newline$1 !== "\r" && newline$1 !== "\r\n") newline$1 = "\n";
			var cursor = 0;
			var aborted = false;
			this.parse = function(input, baseIndex, ignoreLastRow) {
				if (typeof input !== "string") throw new Error("Input must be a string");
				var inputLen = input.length, delimLen = delim.length, newlineLen = newline$1.length, commentsLen = comments.length;
				var stepIsFunction = isFunction(step);
				cursor = 0;
				var data = [], errors = [], row = [], lastCursor = 0;
				if (!input) return returnable();
				if (fastMode || fastMode !== false && input.indexOf(quoteChar) === -1) {
					var rows = input.split(newline$1);
					for (var i = 0; i < rows.length; i++) {
						row = rows[i];
						cursor += row.length;
						if (i !== rows.length - 1) cursor += newline$1.length;
				...
				var nextNewline = input.indexOf(newline$1, cursor);
				...
				for (;;) {
					if (input[cursor] === quoteChar) {
						...
						for (;;) {
							...
							if (quoteSearch === inputLen - 1) return finish$1(input.substring(cursor, quoteSearch).replace(quoteCharRegex, quoteChar));
							...
							if (nextNewline !== -1 && nextNewline < quoteSearch + 1) nextNewline = input.indexOf(newline$1, quoteSearch + 1);
							...
							if (input.substr(quoteSearch + 1 + spacesBetweenQuoteAndDelimiter, delimLen) === delim) {
								...
								nextNewline = input.indexOf(newline$1, cursor);
								break;
							}
							var spacesBetweenQuoteAndNewLine = extraSpaces(nextNewline);
							if (input.substring(quoteSearch + 1 + spacesBetweenQuoteAndNewLine, quoteSearch + 1 + spacesBetweenQuoteAndNewLine + newlineLen) === newline$1) {
								row.push(input.substring(cursor, quoteSearch).replace(quoteCharRegex, quoteChar));
				...
				*/
				...
				function returnable(stopped) {
					if (config.header && !baseIndex && data.length && !headerParsed) {
						...
						for (let i$1 = 0; i$1 < result.length; i$1++) {
							let header = stripBom(result[i$1]);
							if (isFunction(config.transformHeader)) header = config.transformHeader(header, i$1);
		...
		function newWorker() {
			if (!Papa$1.WORKERS_SUPPORTED) return false;
		...
		function workerThreadReceivedMessage(e) {
			var msg = e.data;
			if (typeof Papa$1.WORKER_ID === "undefined" && msg) Papa$1.WORKER_ID = msg.workerId;
			if (typeof msg.input === "string") global.postMessage({
				workerId: Papa$1.WORKER_ID,
				results: Papa$1.parse(msg.input, msg.config),
				finished: true
			});
			else if (global.File && msg.input instanceof File || msg.input instanceof Object) {
				var results = Papa$1.parse(msg.input, msg.config);
		...
		function bindFunction(f, self$1) {
			return function() {
				f.apply(self$1, arguments);
			};
		}
...
//#endregion
//#region src/client/data-preview.ts
var import_papaparse = /* @__PURE__ */ __toESM(require_papaparse(), 1);
const MAX_TABLE_ROWS = 1e4;
const TEXT_PAGE_SIZE = 250;
...
function parseTable(content, delimiter) {
	...
	if (parsed.errors.length > 0) return {
		kind: "error",
		message: parsed.errors[0]?.message ?? "Invalid table data"
	};
	...
	return {
		...
		rows: rawRows.slice(0, MAX_TABLE_ROWS).map((row) => Array.from({ length: width }, (_, index) => row[index] ?? "")),
		truncated: rawRows.length > MAX_TABLE_ROWS
	};
}
function parseDataPreview(path, content) {
...
function compareCells(left, right) {
	...
	if (left.trim() !== "" && right.trim() !== "" && Number.isFinite(leftNumber) && Number.isFinite(rightNumber)) return leftNumber - rightNumber;
...
function sortTableRows(rows, column, direction) {
	...
	return [...rows].sort((left, right) => compareCells(left[column] ?? "", right[column] ?? "") * sign);
}
...
function collectJsonDescendantPaths(value, path, output) {
	output.add(path);
	if (value === null || typeof value !== "object") return;
	for (const [key, child] of Object.entries(value)) collectJsonDescendantPaths(child, `${path}.${key}`, output);
}
/** Return matching JSON nodes and their ancestors in one traversal. */
function collectJsonMatchPaths(value, rawQuery) {
	const query = rawQuery.trim().toLocaleLowerCase();
	const output = /* @__PURE__ */ new Set();
	if (query === "") return output;
	const visit = (current, path) => {
		if (current === null || typeof current !== "object") {
			const matches$1 = String(current).toLocaleLowerCase().includes(query);
			if (matches$1) output.add(path);
			return matches$1;
		}
		let matches = false;
...
function searchTextLines(content, query) {
	const lines = content.split(/\r?\n/);
	const needle = query.trim().toLocaleLowerCase();
	return {
		lines,
		matches: needle === "" ? [] : lines.flatMap((line, index) => line.toLocaleLowerCase().includes(needle) ? [index] : [])
	};
}

//#endregion
//#region src/client/DataFilePreview.tsx
...
function SearchField(props) {
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
		className: "dsh-od-data-search",
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$3.Search, {
			size: 14,
			"aria-hidden": true
...
function collectJsonBranches(value, path = "$", output = /* @__PURE__ */ new Set()) {
	if (value === null || typeof value !== "object" || output.size >= 2e3) return output;
	output.add(path);
	for (const [key, child] of Object.entries(value)) collectJsonBranches(child, `${path}.${key}`, output);
	return output;
}
function JsonNode(props) {
	if (props.matchingPaths !== null && !props.matchingPaths.has(props.path)) return null;
	if (!(props.value !== null && typeof props.value === "object")) return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		className: "dsh-od-json-row",
		style: { "--dsh-json-depth": props.depth },
		children: [
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { className: "dsh-od-json-spacer" }),
			props.name !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				className: "dsh-od-json-key",
				children: props.name
			}),
			props.name !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				className: "dsh-od-json-colon",
				children: ":"
			}),
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				className: `dsh-od-json-value dsh-od-json-${props.value === null ? "null" : typeof props.value}`,
				children: jsonLabel(props.value)
			})
		]
	});
	const entries$1 = Object.entries(props.value);
	const open = props.matchingPaths !== null || props.expanded.has(props.path);
	const summary = Array.isArray(props.value) ? `${entries$1.length} items` : `${entries$1.length} keys`;
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		className: "dsh-od-json-branch",
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
			...
			children: [
				open ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$3.ChevronDown, {
					size: 14,
					"aria-hidden": true
				}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$3.ChevronRight, {
					size: 14,
					"aria-hidden": true
				}),
				props.name !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					className: "dsh-od-json-summary",
					children: summary
				})
			]
		}), open && entries$1.map(([key, child]) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(JsonNode, {
			name: key,
			value: child,
			path: `${props.path}.${key}`,
			depth: props.depth + 1,
...
function JsonPreview(props) {
	...
	const [rawMode, setRawMode] = (0, react.useState)(false);
	const [expanded, setExpanded] = (0, react.useState)(() => new Set(["$"]));
	const allBranches = (0, react.useMemo)(() => collectJsonBranches(props.value), [props.value]);
	const formatted = (0, react.useMemo)(() => JSON.stringify(props.value, null, 2), [props.value]);
	const normalizedQuery = query.trim().toLocaleLowerCase();
	const matchingPaths = (0, react.useMemo)(() => normalizedQuery === "" ? null : collectJsonMatchPaths(props.value, normalizedQuery), [normalizedQuery, props.value]);
	const toggle = (path) => {
		setExpanded((current) => {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
		className: "dsh-od-data-preview",
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
			className: "dsh-od-data-toolbar",
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(SearchField, {
				value: query,
				label: props.labels.search,
				onChange: setQuery
			}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "dsh-od-data-actions",
				children: [!rawMode && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(PreviewButton, {
					...
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$3.ChevronsUpDown, {
						size: 15,
						"aria-hidden": true
					})
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(PreviewButton, {
					label: rawMode ? props.labels.structured : props.labels.raw,
					active: rawMode,
					onClick: () => {
						setRawMode((current) => !current);
					},
					children: rawMode ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$3.Braces, {
						size: 15,
						"aria-hidden": true
					}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$3.AlignLeft, {
						size: 15,
						"aria-hidden": true
					})
				})]
			})]
		}), rawMode ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("pre", {
			className: "dsh-od-data-raw",
			children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", { children: formatted })
		}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
			className: "dsh-od-json-tree",
...
function TablePreview(props) {
	...
	const [sort, setSort] = (0, react.useState)(null);
	const [widths, setWidths] = (0, react.useState)({});
	const filtered = (0, react.useMemo)(() => filterTableRows(props.rows, query), [props.rows, query]);
	const paged = paginateRows((0, react.useMemo)(() => sort === null ? filtered : sortTableRows(filtered, sort.column, sort.direction), [filtered, sort]), page, 100);
	(0, react.useEffect)(() => {
		setPage((current) => Math.min(current, paged.pageCount));
	}, [paged.pageCount]);
	const resize = (column, event) => {
		...
		const move = (next) => {
			setWidths((current) => ({
				...current,
				[column]: Math.max(88, startWidth + next.clientX - startX)
			}));
		};
		const stop = () => {
		...
		children: [
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						className: "dsh-od-data-count",
						children: props.labels.rows(filtered.length)
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(PreviewButton, {
						label: rawMode ? props.labels.structured : props.labels.raw,
						active: rawMode,
			...
			props.truncated && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: "dsh-od-data-notice",
				children: props.labels.truncated(MAX_TABLE_ROWS)
			}),
			rawMode ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("pre", {
				className: "dsh-od-data-raw",
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", { children: props.raw })
			}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: "dsh-od-table-scroll",
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("table", {
					className: "dsh-od-data-table",
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("colgroup", { children: props.columns.map((_, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("col", { style: { width: widths[index] ?? 160 } }, index)) }),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("thead", { children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("tr", { children: props.columns.map((column, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("th", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
							type: "button",
							onClick: () => {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: column }), sort?.column === index && (sort.direction === "asc" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$3.ArrowUp, {
								size: 12,
								"aria-hidden": true
							}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$3.ArrowDown, {
								size: 12,
								"aria-hidden": true
							}))]
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: "dsh-od-column-resize",
							role: "separator",
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("tbody", { children: paged.rows.map((row, rowIndex) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("tr", { children: props.columns.map((_, columnIndex) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("td", {
							title: row[columnIndex] ?? "",
							children: row[columnIndex] ?? ""
						}, columnIndex)) }, (paged.page - 1) * 100 + rowIndex)) })
					]
				})
			}),
			!rawMode && paged.pageCount > 1 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "dsh-od-data-pagination",
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(PreviewButton, {
						...
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$3.ChevronLeft, {
							size: 15,
							"aria-hidden": true
						})
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: props.labels.page(paged.page, paged.pageCount) }),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(PreviewButton, {
						...
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$3.ChevronRight, {
							size: 15,
							"aria-hidden": true
						})
					})
...
function TextReader(props) {
	...
	const [page, setPage] = (0, react.useState)(1);
	const search = (0, react.useMemo)(() => searchTextLines(props.content, query), [props.content, query]);
	const matches = (0, react.useMemo)(() => new Set(search.matches), [search.matches]);
	const paged = paginateRows((0, react.useMemo)(() => {
		return (query.trim() === "" ? search.lines.map((_, index) => index) : search.matches).map((index) => ({
			index,
			text: search.lines[index] ?? ""
		}));
	}, [
		query,
		search.lines,
		...
		children: [
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					query.trim() !== "" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						className: "dsh-od-data-count",
						children: props.labels.matches(matches.size)
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(PreviewButton, {
						label: props.labels.wrap,
						active: wrap,
						onClick: () => {
							setWrap((current) => !current);
						},
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$3.WrapText, {
							size: 15,
							"aria-hidden": true
						})
					})
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: "dsh-od-text-reader",
				"data-wrap": wrap || void 0,
				children: paged.rows.map((line) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: "dsh-od-text-line",
					"data-match": matches.has(line.index) || void 0,
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: line.index + 1 }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", { children: line.text || " " })]
				}, line.index))
			}),
			paged.pageCount > 1 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "dsh-od-data-pagination",
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(PreviewButton, {
						label: props.labels.previous,
...
function DataFilePreview(props) {
	const preview = (0, react.useMemo)(() => parseDataPreview(props.path, props.content), [props.content, props.path]);
	if (preview.kind === "error") return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		className: "dsh-od-preview-state",
		"data-state": "error",
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: props.labels.parseError }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: preview.message })]
	});
	if (preview.kind === "json") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(JsonPreview, {
		value: preview.value,
		labels: props.labels
	});
	if (preview.kind === "table") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(TablePreview, {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(TextReader, {
...
//#endregion
//#region src/client/media-preview.ts
function adjustZoom(current, direction) {
	const base = current === "fit" ? 1 : current;
	const factor = direction === 1 ? 1.25 : .8;
	return Math.min(4, Math.max(.25, Number((base * factor).toFixed(3))));
}
...
function svgIntrinsicSize(source) {
	...
	const viewBox = svg$2.getAttribute("viewBox")?.trim().split(/[\s,]+/).map(Number);
	if (viewBox?.length === 4 && viewBox.every(Number.isFinite) && (viewBox[2] ?? 0) > 0 && (viewBox[3] ?? 0) > 0) return {
		width: viewBox[2] ?? 0,
		height: viewBox[3] ?? 0
	};
...
//#endregion
//#region src/client/MediaPreview.tsx
...
function MediaPreview(props) {
	...
	const svgSize = (0, react.useMemo)(() => props.kind === "svg" ? svgIntrinsicSize(props.source) : null, [props.kind, props.source]);
	const [imageSize, setImageSize] = (0, react.useState)(null);
	const [zoom, setZoom] = (0, react.useState)("fit");
	const [checker, setChecker] = (0, react.useState)(false);
	const size = props.kind === "svg" ? svgSize : imageSize;
	const scaleLabel = zoom === "fit" ? null : `${Math.round(zoom * 100)}%`;
	const contentStyle = zoom === "fit" ? {} : size === null ? { width: `${zoom * 100}%` } : {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
		className: "dsh-od-media-preview",
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
			className: "dsh-od-media-toolbar",
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "dsh-od-media-meta",
				children: [size !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: props.labels.dimensions(Math.round(size.width), Math.round(size.height)) }), scaleLabel !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: scaleLabel })]
			}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "dsh-od-data-actions",
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(ToolButton, {
						label: props.labels.zoomOut,
						onClick: () => {
							setZoom((current) => adjustZoom(current, -1));
						},
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$2.ZoomOut, {
							size: 15,
							"aria-hidden": true
						})
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(ToolButton, {
						...
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$2.ZoomIn, {
							size: 15,
							"aria-hidden": true
						})
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(ToolButton, {
						label: props.labels.fit,
						active: zoom === "fit",
						onClick: () => {
							setZoom("fit");
						},
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$2.Maximize2, {
							size: 15,
							"aria-hidden": true
						})
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(ToolButton, {
						label: props.labels.actualSize,
						active: zoom === 1,
						onClick: () => {
							setZoom(1);
						},
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$2.Scan, {
							size: 15,
							"aria-hidden": true
						})
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(ToolButton, {
						label: props.labels.transparency,
						active: checker,
						onClick: () => {
							setChecker((current) => !current);
						},
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$2.Grid2X2, {
							size: 15,
							"aria-hidden": true
						})
					})
			...
			ref: stageRef,
			className: "dsh-od-media-stage",
			"data-checker": checker || void 0,
			"data-pannable": zoom !== "fit" || void 0,
			onPointerDown: beginPan,
			children: props.kind === "svg" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: "dsh-od-media-content dsh-od-media-svg",
				style: contentStyle,
				dangerouslySetInnerHTML: { __html: props.source }
			}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("img", {
				className: "dsh-od-media-content",
				style: contentStyle,
				src: props.source,
				alt: props.alt,
				onLoad: (event) => {
...
async function outputFileSize(path) {
	try {
...
function prepareSvg(_sourcePath, source) {
	...
	const numericDimension$1 = (value) => {
		const match = /^\s*(\d+(?:\.\d+)?)\s*(?:px)?\s*$/i.exec(value ?? "");
		if (match === null) return null;
		const parsed = Number(match[1]);
		return Number.isFinite(parsed) && parsed > 0 ? parsed : null;
	...
	if (!svg$2.hasAttribute("viewBox")) {
		const width = numericDimension$1(svg$2.getAttribute("width"));
		const height = numericDimension$1(svg$2.getAttribute("height"));
...
function useFileAccess(entry) {
	...
	(0, react.useEffect)(() => {
		...
		authorizeFileContent(entry.path).then((source) => {
			if (!stale) setState({
				status: "ready",
				source
			});
...
function useTextPreview(entry, access) {
	...
	(0, react.useEffect)(() => {
		...
		fetch(fileUrl(access.source, entry.lastSeq)).then(async (response) => {
			if (!response.ok) throw new Error(String(response.status));
...
function MarkdownPreview({ source, content, onSaveState }) {
	const html$3 = (0, react.useMemo)(() => prepareHtmlFragment(source, sanitizeHtml(marked.parse(content, { async: false }))), [content, source]);
	...
	const onInput = (event) => {
		...
		timer.current = window.setTimeout(() => {
			onSaveState?.("saving");
			saveFileContent(source, markdown).then(() => {
	...
	if (isNetworkOutput(source)) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
...
function SvgPreview({ entry, source, content, labels }) {
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(MediaPreview, {
		kind: "svg",
		source: (0, react.useMemo)(() => prepareSvg(source, sanitizeSvg(content)), [content, source]),
		alt: entry.path,
		labels,
		onLoad: () => {},
		onError: () => {}
...
function ImagePreview({ entry, source, onResult, labels }) {
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(MediaPreview, {
		kind: "image",
		source: fileUrl(source, entry.lastSeq),
		alt: entry.path,
		labels,
		onLoad: () => {
			onResult({
...
function HtmlPreview({ entry, source, content }) {
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("iframe", {
		className: "dsh-od-preview-frame",
		sandbox: "",
		srcDoc: (0, react.useMemo)(() => prepareHtml(source, content), [content, source]),
		title: entry.path
	});
}
function codeLanguage(path) {
	const extension$2 = path.slice(path.lastIndexOf(".") + 1).toLowerCase();
	return {
		js: "JavaScript",
		jsx: "JSX",
		ts: "TypeScript",
		...
		vue: "Vue",
		svelte: "Svelte"
	}[extension$2] ?? extension$2.toUpperCase();
}
		...
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
			...
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
					...
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$1.RefreshCw, {
						size: 15,
						"aria-hidden": true
					})
					...
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$1.ExternalLink, {
						size: 15,
						"aria-hidden": true
					})
				})]
...
function MediaFilePreview({ entry, source, kind, onResult, labels }) {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
		...
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		}), kind === "video" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("video", {
			className: "dsh-od-preview-media",
			src,
			controls: true,
			preload: "metadata",
			onLoadedMetadata: () => {
			...
			className: "dsh-od-audio-stage",
			children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("audio", {
				className: "dsh-od-preview-audio",
				src,
				controls: true,
				preload: "metadata",
				onLoadedMetadata: () => {
...
function Preview({ entry, onResult, onSaveState, labels }) {
	...
	(0, react.useEffect)(() => {
		...
		let stale = false;
		const run = async () => {
			const result = entry.kind === "md" ? await checkMarkdown(access.status === "ready" ? access.source : entry.path, state.content) : entry.kind === "svg" ? await checkSvg(state.content) : entry.kind === "html" ? await checkHtml(state.content) : {
				level: "ok",
				issues: []
			};
	...
	if (access.status === "error" || state.status === "error") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
		className: "dsh-od-preview-state",
		"data-state": "error",
		children: labels.error
	});
	if (access.status !== "ready" || isTextBacked(entry) && state.status !== "ready") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
		className: "dsh-od-preview-state",
		"data-state": "loading",
	...
	switch (entry.kind) {
		...
		case "video": return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(MediaFilePreview, {
			entry,
			source: access.source,
			kind: "video",
			onResult,
		...
		case "audio": return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(MediaFilePreview, {
			entry,
			source: access.source,
			kind: "audio",
			onResult,
		...
		case "text": return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(DataFilePreview, {
			path: entry.path,
			content: state.status === "ready" ? state.content : "",
			labels: labels.data
		});
...
//#endregion
//#region src/client/qc-labels.ts
const ISSUE_KEYS = {
...
function filterByGroup(entries$1, group) {
	if (group === "all") return entries$1;
	return entries$1.filter((entry) => kindGroup(entry.kind) === group);
}
...
function formatBytes(bytes) {
	...
	return `${unit === 0 || value >= 10 ? String(Math.round(value)) : value.toFixed(1)} ${units[unit]}`;
}
...
function groupCatalogByTurn(entries$1) {
	const groups = /* @__PURE__ */ new Map();
	for (const entry of entries$1) {
		const bucket = groups.get(entry.firstTurn);
		if (bucket === void 0) groups.set(entry.firstTurn, [entry]);
		else bucket.push(entry);
	}
	return [...groups.entries()].sort((left, right) => right[0] - left[0]).map(([turn, list$2]) => ({
		turn,
		entries: [...list$2].sort((left, right) => right.lastSeq - left.lastSeq)
	}));
}
/** Case-insensitive catalog filter across file name and full path. */
function filterCatalog(entries$1, query) {
	const needle = query.trim().toLocaleLowerCase();
	if (needle === "") return entries$1;
	return entries$1.filter((entry) => entry.path.toLocaleLowerCase().includes(needle));
}
/** Close every listed tab at its current revision so nothing reopens by itself. */
function closedAllAt(entries$1) {
	const closedAt = {};
	for (const entry of entries$1) closedAt[entry.path] = entry.lastSeq;
	return closedAt;
}
...
function OutputDockSurface(props) {
	...
	const [qcOpen, setQcOpen] = (0, react.useState)(false);
	...
	const [catalogGroup, setCatalogGroup] = (0, react.useState)("all");
	const [hiddenOpen, setHiddenOpen] = (0, react.useState)(false);
	const [sizes, setSizes] = (0, react.useState)(/* @__PURE__ */ new Map());
	const sizesRef = (0, react.useRef)(sizes);
	const [saveState, setSaveState] = (0, react.useState)(null);
	const [resizing, setResizing] = (0, react.useState)(false);
	const [liveWidth, setLiveWidth] = (0, react.useState)(null);
	const selectedRef = (0, react.useRef)(null);
	const seenSeqRef = (0, react.useRef)(0);
	const liveWidthRef = (0, react.useRef)(null);
	...
	const hiddenEntries = (0, react.useMemo)(() => snapshot.entries.filter((entry) => hidden.has(entry.path)), [hidden, snapshot.entries]);
	const grouped = (0, react.useMemo)(() => filterByGroup(catalog, catalogGroup), [catalog, catalogGroup]);
	const filtered = (0, react.useMemo)(() => filterCatalog(grouped, catalogQuery), [grouped, catalogQuery]);
	const groups = (0, react.useMemo)(() => groupCatalogByTurn(filtered), [filtered]);
	...
	(0, react.useEffect)(() => {
		let stale = false;
		Promise.all(snapshot.entries.map(async (entry) => {
			...
			for (const update of fresh) next.set(update.path, {
				seq: update.seq,
				bytes: update.bytes
			});
	...
	const qc = selectedPath === null ? void 0 : qcByPath.get(selectedPath);
	const select = (path) => {
	...
	const onSave = (0, react.useCallback)((state) => {
		setSaveState(state);
	}, []);
	...
	const reveal = (entry) => {
		...
		authorizeFileContent(entry.path).then((source) => {
			props.openPath(directoryOfPath(source));
		}).catch(() => {});
	};
	...
	const unhide = (path) => {
		setPersisted((previous) => ({
			...previous,
			hidden: previous.hidden.filter((item) => item !== path)
		}));
	};
	const clearHidden = () => {
		setPersisted((previous) => ({
	...
	const startResize = (event) => {
		...
		const onUp = () => {
			...
			setLiveWidth(null);
			if (next !== null) setPersisted((previous) => ({
				...previous,
				width: next
			}));
		};
	...
	(0, react.useEffect)(() => {
		const onKey = (event) => {
			...
			if (event.code === "BracketRight" || event.code === "BracketLeft") {
				...
				const next = visible[(visible.findIndex((entry) => entry.path === selectedPath) + step + visible.length) % visible.length];
				if (next !== void 0) select(next.path);
				return;
			}
			if (event.code === "KeyW") {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
		...
		children: [
				...
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(Preview, {
					...
					labels: {
						...
						data: {
							...
							next: t("preview.next"),
							page: (page, count) => t("preview.page", {
								page,
								count
							}),
						...
						media: {
							...
							dimensions: (width$1, height) => t("preview.dimensions", {
								width: width$1,
								height
							})
						},
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("footer", {
				...
				children: [
					hiddenOpen && hiddenEntries.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "dsh-od-catalog",
						role: "listbox",
						"aria-label": t("dock.clearHidden"),
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								className: "dsh-od-catalog-group",
								children: t("dock.hiddenCount", { count: hiddenCount })
							}),
							hiddenEntries.map((entry) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)(IconButton, {
										...
										children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react.Eye, {
											size: 16,
											"aria-hidden": true
										})
									})
					...
					qcOpen && qc !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "dsh-od-qc",
						"aria-live": "polite",
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: "dsh-od-qc-title",
							children: t("qc.title")
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("ul", {
							className: "dsh-od-qc-list",
							children: qc.issues.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("li", { children: t("qc.ok") }) : qc.issues.map((issue, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("li", {
								"data-level": issue.level,
								children: t(qcIssueKey(issue.code), issue.count === void 0 ? void 0 : { count: issue.count })
							}, `${issue.code}:${index}`))
						})]
					}),
					catalogOpen && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: "dsh-od-catalog-tools",
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
									className: "dsh-od-catalog-search",
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react.Search, {
										size: 14,
										"aria-hidden": true
									}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
										type: "search",
									...
									className: "dsh-od-catalog-filters",
									role: "group",
									...
									children: [
										...
										"data-active": catalogGroup === key || void 0,
										"aria-pressed": catalogGroup === key,
										onClick: () => {
											setCatalogGroup(key);
										},
										children: [t(FILTER_KEYS[key]), key !== "all" && (groupCounts.get(key) ?? 0) > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("small", { children: groupCounts.get(key) }) : null]
									}, key))
								})]
							}),
							groups.length === 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								className: "dsh-od-catalog-group",
								children: t("dock.noMatch")
							}),
							groups.map((group) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								role: "group",
								"aria-label": t("dock.turn", { turn: group.turn }),
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									className: "dsh-od-catalog-group",
									children: t("dock.turn", { turn: group.turn })
								}), group.entries.map((entry) => {
									const size = sizes.get(entry.path);
									const sizeText = size?.bytes === void 0 || size.bytes === null ? "" : ` · ${formatBytes(size.bytes)}`;
									return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
										...
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
											...
											className: "dsh-od-catalog-copy",
											children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: basename(entry.path) }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("small", { children: [entry.path, sizeText] })]
										})]
									}, entry.path);
								})]
							}, group.turn)),
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							hiddenCount > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(IconButton, {
								label: t("dock.hiddenCount", { count: hiddenCount }),
								active: hiddenOpen,
								onClick: () => {
									setHiddenOpen((current) => !current);
								},
							...
							qc !== void 0 && qc.level !== "loading" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(IconButton, {
								label: qc.issues.length === 0 ? t("qc.ok") : t(qcSummaryKey(qc) ?? "qc.ok", qc.issues[0]?.count === void 0 ? void 0 : { count: qc.issues[0]?.count }),
								active: qcOpen,
								onClick: () => {
									setQcOpen((current) => !current);
								},
								children: qc.level === "ok" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react.ShieldCheck, {
									size: 16,
									"aria-hidden": true
								}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react.AlertTriangle, {
									size: 16,
									"aria-hidden": true
								})
							}),
							saveState === "saving" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: "dsh-od-status",
								children: t("dock.saving")
							}),
							...
							saveState === "error" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: "dsh-od-status",
								"data-state": "error",
								children: t("dock.saveError")
							}),
							selected !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)(IconButton, {
										...
										disabled: selected.kind === "image" || selected.kind === "pdf" || selected.kind === "video" || selected.kind === "audio",
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)(IconButton, {
										label: isNetworkOutput(selected.path) ? t("preview.openExternal") : t("dock.reveal"),
										onClick: () => {
											reveal(selected);
										},
										children: isNetworkOutput(selected.path) ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react.ExternalLink, {
											size: 16,
											"aria-hidden": true
										}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react.FolderOpen, {
											size: 16,
											"aria-hidden": true
										})
									}),
...
const zh = {
	...
	"dock.filterData": "数据",
	"dock.filterMedia": "媒体",
	"dock.closeAll": "关闭全部标签",
	"dock.resize": "拖动调整侧栏宽度",
...
const en = {
	...
	"dock.copied": "Copied",
	"dock.saved": "Saved",
	"dock.search": "Search outputs",
	"dock.noMatch": "No matching outputs",
	"dock.filter": "Filter by type",
	"dock.filterAll": "All",
	"dock.filterDoc": "Docs",
	"dock.filterImage": "Images",
	"dock.filterData": "Data",
	"dock.filterMedia": "Media",
	"dock.closeAll": "Close all tabs",
	"dock.resize": "Drag to resize the sidebar",
	...
	"preview.video": "Video",
	"preview.audio": "Audio",
	"qc.title": "Output checks",
	"qc.mediaFailed": "Media failed to play",
	"qc.ok": "Checks passed",
...
function samePathVariant(left, right) {
	if (isNetworkOutput(left) || isNetworkOutput(right)) return left.toLowerCase() === right.toLowerCase();
	const a = normalizedPath(left);
	const b = normalizedPath(right);
	return a === b || a.endsWith(`/${b}`) || b.endsWith(`/${a}`);
}
function canonicalPath(left, right) {
	if (isNetworkOutput(left) || isNetworkOutput(right)) return right;
	const leftAbsolute = /^[A-Za-z]:[\\/]|^\//.test(left);
	if (leftAbsolute !== /^[A-Za-z]:[\\/]|^\//.test(right)) return leftAbsolute ? left : right;
	...
	snapshot() {
		...
		for (const turn of order) {
			...
			for (const produced of payload.produced) {
				...
				const previousKey = [...entries$1.keys()].find((path$1) => samePathVariant(path$1, produced.path));
				const previous = previousKey === void 0 ? void 0 : entries$1.get(previousKey);
				const path = previousKey === void 0 ? produced.path : canonicalPath(previousKey, produced.path);
				if (previousKey !== void 0 && previousKey !== path) entries$1.delete(previousKey);
				entries$1.set(path, previous === void 0 ? {
					path,
					kind,
					firstTurn: turn,
					lastTurn: turn,
