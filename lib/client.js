...
//#region src/publication.ts
const OUTPUT_DOCK_TOOL = "output_dock_publish";
...
function requiredText(record, key, maxLength) {
...
function normalizePublication(input) {
	if (typeof input !== "object" || input === null || Array.isArray(input)) throw new Error("publication must be an object");
	const record = input;
	const unknownKey = Object.keys(record).find((key) => !INPUT_KEYS.has(key));
	if (unknownKey !== void 0) throw new Error(`unknown publication field: ${unknownKey}`);
	...
	if (kind !== "document" && kind !== "visual" && kind !== "link") throw new Error("kind must be document, visual, or link");
	...
	if (hasUrl) {
		...
		if (parsed.protocol !== "http:" && parsed.protocol !== "https:") throw new Error("link URL must use HTTP or HTTPS");
		return {
			workId,
			workTitle,
			resultId,
...
function publicationFromStart(match) {
	try {
		if (match.event.type === "tool/call") return normalizePublication(JSON.parse(match.event.data.arguments));
		if (match.event.type === "tool/code-dispatch-start") return normalizePublication(match.event.data.arguments);
	} catch {
		return null;
	}
	return null;
}
function succeeded(state, match) {
	if (state.mode === "native" && match.event.type === "tool/result") return match.event.data.message.content[0].isError === false;
	if (state.mode === "code" && match.event.type === "tool/code-dispatch") return match.event.data.name === OUTPUT_DOCK_TOOL && match.event.data.isError === false;
	return false;
}
function nodeFor(context) {
	const state = context.state;
	if (state?.publication === null || state?.publication === void 0 || state.settledSeq === void 0) return null;
	return {
		key: context.key,
		kind: context.kind,
		id: context.id,
...
const outputDockDefinition = {
	...
	target: "outputDock",
	match(event) {
		if (event.type === "tool/call" && event.data.name === OUTPUT_DOCK_TOOL) return {
			id: `native:${event.data.callId}`,
			role: "start"
		};
		if (event.type === "tool/result") return {
			id: `native:${event.data.message.source.callId}`,
			role: "update"
		};
		if (event.type === "tool/code-dispatch-start" && event.data.name === OUTPUT_DOCK_TOOL) return {
			id: `code:${event.data.subCallId}`,
			role: "start"
		};
...
var require_lucide_react = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	const Copy$1 = createLucideIcon("Copy", [["rect", {
		width: "14",
		height: "14",
		x: "8",
		y: "8",
	...
	const RotateCw$2 = createLucideIcon("RotateCw", [["path", {
	...
	exports.RotateCw = RotateCw$2;
	exports.X = X$1;
}));

//#endregion
//#region src/client/contract.ts
var import_lucide_react$2 = require_lucide_react();
const EMPTY_OUTPUT_DOCK_SNAPSHOT = {
	entries: [],
	history: []
};
function publicationKey(workId, resultId) {
	return `${workId}\u0000${resultId}`;
}
...
function fileUrl(path, revision = 0) {
...
function resolveResourceUrl(sourcePath, href, revision = 0) {
	...
	return `${fileUrl(normalizeSibling(sourcePath, reference), revision)}${query === "" ? "" : `&${query}`}${hash}`;
}
/** Normalize a sanitized SVG document for a contained sidebar preview. */
function prepareSvg(_sourcePath, source, revision = 0) {
...
function rewriteHtmlResources(sourcePath, doc, revision) {
...
function prepareHtml(sourcePath, source, revision = 0) {
...
function prepareHtmlFragment(sourcePath, source, revision = 0) {
	const doc = new DOMParser().parseFromString(source, "text/html");
	rewriteHtmlResources(sourcePath, doc, revision);
...
function isTextBacked(entry) {
	return entry.previewKind === "md" || entry.previewKind === "svg" || entry.previewKind === "html" || entry.previewKind === "text";
}
function useTextPreview(entry) {
	...
	(0, react.useEffect)(() => {
		...
		fetch(fileUrl(entry.path, entry.revision)).then(async (response) => {
...
function MarkdownPreview({ entry, content }) {
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
		...
		dangerouslySetInnerHTML: { __html: (0, react.useMemo)(() => prepareHtmlFragment(entry.path, sanitizeHtml(marked.parse(content, { async: false })), entry.revision), [
			content,
			entry.path,
			entry.revision
		]) }
...
function SvgPreview({ entry, content }) {
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
		...
		dangerouslySetInnerHTML: { __html: (0, react.useMemo)(() => prepareSvg(entry.path, sanitizeSvg(content), entry.revision), [
			content,
			entry.path,
			entry.revision
		]) }
...
function HtmlPreview({ entry, content }) {
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("iframe", {
		...
		sandbox: "",
		srcDoc: (0, react.useMemo)(() => prepareHtml(entry.path, content, entry.revision), [
			content,
			entry.path,
			entry.revision
		]),
...
function Unavailable({ labels, onRefresh }) {
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		"data-state": "error",
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: labels.unavailable }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
			type: "button",
			className: "dsh-od-text-btn",
			onClick: onRefresh,
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$2.RotateCw, {
				size: 14,
				"aria-hidden": true
			}), labels.retry]
		})]
...
function Preview({ entry, labels, onRefresh }) {
	const state = useTextPreview(entry);
	const [binaryFailed, setBinaryFailed] = (0, react.useState)(false);
	if (state.status === "error" || binaryFailed) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(Unavailable, {
		labels,
		onRefresh
	});
	...
	switch (entry.previewKind) {
		...
		case "pdf": return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("iframe", {
...
var import_lucide_react$1 = require_lucide_react();
const OUTPUT_SURFACE = "output-dock";
function isPreviewEntry(entry) {
	return entry.kind !== "link" && typeof entry.path === "string" && entry.previewKind !== null;
}
function dockTabs(state, snapshot) {
...
function useDockState(store, sessionId) {
	const subscribe = (0, react.useCallback)((onChange) => store.subscribe(sessionId, onChange), [sessionId, store]);
	const getSnapshot = (0, react.useCallback)(() => store.get(sessionId), [sessionId, store]);
	return (0, react.useSyncExternalStore)(subscribe, getSnapshot, getSnapshot);
}
...
function OutputDockSurface(props) {
	...
	const model = (0, react.useMemo)(() => dockTabs(state, snapshot), [snapshot, state]);
	const [refreshByKey, setRefreshByKey] = (0, react.useState)(/* @__PURE__ */ new Map());
	...
	const close = (0, react.useCallback)((key) => {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
		...
		"aria-label": props.t("dock.title"),
		children: [
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", {
				className: "dsh-od-header",
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						className: "dsh-od-title",
						children: props.t("dock.title")
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: "dsh-od-tabs",
						role: "tablist",
						"aria-label": props.t("dock.tabs"),
						children: model.tabs.map((tab) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: "dsh-od-tab-wrap",
							"data-active": tab.key === active.key || void 0,
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
								...
								onClick: () => {
									props.dockStore.activate(current.id, tab.key);
								},
								children: [
									tab.workTitle,
									" · ",
								...
								children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$1.X, {
									size: 13,
									"aria-hidden": true
								})
							})]
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(IconButton, {
						...
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$1.PanelRightClose, {
							size: 16,
							"aria-hidden": true
						})
					})
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "dsh-od-resource-bar",
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					className: "dsh-od-resource-name",
					children: active.label
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					className: "dsh-od-resource-path",
					title: active.path,
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				}, `${active.key}:${active.revision}:${manualRefresh}`)
			}),
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("footer", {
				className: "dsh-od-toolbar",
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(IconButton, {
						label: props.t("dock.refresh"),
						onClick: refresh,
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$1.RotateCw, {
							size: 16,
							"aria-hidden": true
						})
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(IconButton, {
						label: copied ? props.t("dock.copied") : props.t("dock.copyPath"),
						onClick: () => {
							copyPath();
						},
						children: copied ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$1.Check, {
							size: 16,
							"aria-hidden": true
						}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$1.Copy, {
							size: 16,
							"aria-hidden": true
						})
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(IconButton, {
						label: props.t("dock.download"),
						onClick: download,
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$1.Download, {
							size: 16,
							"aria-hidden": true
						})
					}),
...
function OutputDockLauncher(props) {
	...
	const current = currentSession(props);
	const sessionIds = props.useSessions((state$1) => state$1.ids);
	const snapshot = useDockSnapshot(current?.session);
	const state = useDockState(props.dockStore, current?.id ?? "");
	const surface = useDetailsSurface(props.layout);
	const localKeys = (0, react.useMemo)(() => new Set(snapshot.entries.filter(isPreviewEntry).map((entry) => entry.key)), [snapshot.entries]);
	(0, react.useEffect)(() => {
	...
	if (surface === OUTPUT_SURFACE && state.open) return target === "mobile" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
		...
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react$1.Files, {
			size: 17,
			"aria-hidden": true
		}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: state.opened.length })]
	});
}

//#endregion
//#region src/client/dock-store.ts
...
function isSessionState(value) {
	...
	const state = value;
	return typeof state.open === "boolean" && Array.isArray(state.opened) && state.opened.every((key) => typeof key === "string") && (state.active === null || typeof state.active === "string");
}
function hydrate(storage) {
	try {
		...
		const sessions = {};
		for (const [sessionId, state] of Object.entries(root.sessions)) if (isSessionState(state)) {
			const opened = [...new Set(state.opened)];
			const active = state.active !== null && opened.includes(state.active) ? state.active : opened.at(-1) ?? null;
			sessions[sessionId] = {
				open: state.open && opened.length > 0,
				opened,
				active
			};
		}
...
function createOutputDockUiStore(storage) {
	let sessions = hydrate(storage);
	let observedSessionIds = null;
	const listeners = /* @__PURE__ */ new Map();
	const write = (sessionId, next) => {
		const previous = sessions[sessionId];
		if (next === null && previous === void 0) return;
	...
	const get = (sessionId) => sessions[sessionId] ?? EMPTY_STATE;
	return {
		get,
		subscribe(sessionId, listener) {
			const owned = listeners.get(sessionId) ?? /* @__PURE__ */ new Set();
			owned.add(listener);
		...
		open(sessionId, key) {
			...
			write(sessionId, {
				open: true,
				opened: previous.opened.includes(key) ? previous.opened : [...previous.opened, key],
				active: key
			});
		},
		...
		closeTab(sessionId, key) {
			...
			const active = previous.active === key ? opened[index] ?? opened[index - 1] ?? null : previous.active;
			write(sessionId, {
				...previous,
				opened,
				active
		...
		reconcile(sessionId, localKeys) {
			...
			const active = previous.active !== null && opened.includes(previous.active) ? previous.active : opened.at(-1) ?? null;
			write(sessionId, {
				open: previous.open && opened.length > 0,
				opened,
				active
			});
		},
		...
		noteSessions(ids) {
			...
			const ownedRemoved = removed.filter((id) => sessions[id] !== void 0);
			...
			for (const id of ownedRemoved) for (const listener of listeners.get(id) ?? []) listener();
		}
	};
}
...
function outputFromNode(node) {
	...
	return {
		...
		previewKind: publication.path === void 0 ? null : kindOfPath(publication.path),
...
var OutputDockViewBuilder = class {
	...
	snapshot() {
		const history = [...this.nodes.values()].sort((left, right) => left.anchorSeq - right.anchorSeq).flatMap((node) => {
...
function publicationsForTurn(snapshot, turn, closingSeq) {
	const latest = /* @__PURE__ */ new Map();
	for (const entry of snapshot.history) if (entry.turn === turn && entry.revision <= closingSeq) latest.set(entry.key, entry);
...
//#endregion
//#region src/client/ResultActions.tsx
var import_lucide_react = require_lucide_react();
function selectPublishedOutputs(owner) {
	const entries$1 = publicationsForTurn(owner.views.get("outputDock") ?? EMPTY_OUTPUT_DOCK_SNAPSHOT, owner.turn.turn, owner.seq);
	return entries$1.length === 0 ? null : entries$1;
}
function actionDestination(output) {
...
function ResultIcon({ output }) {
	if (output.kind === "link") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react.ExternalLink, {
		size: 14,
		"aria-hidden": true
	});
	if (output.kind === "visual") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react.Image, {
		size: 14,
		"aria-hidden": true
	});
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(import_lucide_react.FileText, {
		size: 14,
		"aria-hidden": true
	});
}
function ResultActions({ matched, sessionId, dockStore, layout, t }) {
	const id = String(sessionId);
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
		className: "dsh-od-result-actions",
		children: matched.map((output) => {
			const label = `${output.workTitle} · ${output.label}`;
			const destination = actionDestination(output);
			const actionLabel = destination.kind === "browser" ? t("result.openLink", { name: label }) : t("result.openOutput", { name: label });
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
				type: "button",
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(ResultIcon, { output }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: label })]
			}, `${output.key}:${output.revision}`);
		})
	});
}
...
*/
function apply(ctx) {
	const dockStore = createOutputDockUiStore(window.localStorage);
